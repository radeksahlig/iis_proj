...
if(isset($_GET['obj']) && isset($_GET['stav'])){
    ...
    $db->close();
    ?><script>
        window.location = "./zakazky.php?search=&f_akt=akt";
    </script><?php
}
?>
<!DOCTYPE html>
        ...
        <section class="row justify-content-md-center">
            <div class="col col-md-12">
                <div class="card shadow-lg border-dark">
                    <h5 class="card-header">Výpis zakázek</h5>
                    <div class="col col-md-5 mt-4">
                        <a class="float-left badge badge-light" href="./zakazky.php">Všechny zakázky</a>
                        <br>
                        <form method="get" class="mb-5" action="./zakazky.php" enctype="multipart/form-data">
                            ...
                            <div class="custom-control custom-checkbox mt-2">
                                <input class="custom-control-input" type="checkbox" name="f_akt" <?php if(isset($_GET['f_akt'])) echo "checked";?> id="check3" />
                                <label class="custom-control-label" for="check3">Pouze aktuální zakázky</label>
                            </div>
                        </form>
                        <?php
                            $stranka = "./zakazky.php?";
                            ...
                            echo "<span>Výpis zakázek $offset - ".($offset+10) . "</span>";
                            ?>
                    </div>
                    <div class="card-body">
            ...
            if($load_my_objednavka = $db->query($sql)){
                if($load_my_objednavka->num_rows>0){
                    ...
                    echo "
                        <thead>
                            <tr>
                                ...
                                <th scope='col'>Na cěste</th>
                                <th scope='col'>Dodáno</th>
                                <th scope='col'>Reset stavu</th>
                            </tr>
                        </thead>
                    "; 
                    while($row = $load_my_objednavka->fetch_assoc()){
                        ...
                        echo "<td><a class='badge badge-light' href='./zakazky.php?obj=".$row['id']."&stav=rs'>Reset stavu</a></td>";
                        echo "</tr>";
                    }
                    echo "</tbody></table>";                
                }else{
                    echo "<p class='alert alert-danger text-center border-danger my-4'>Nepodařilo se načíst žádné objednávky!</p>";
                }
